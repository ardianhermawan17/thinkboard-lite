...
export function annotationCandidates(annotations: PdfAnnotation[], viewport: ViewportLike, rotation: Rotation, textBoxes: TextItemBox[]): ImportCandidate[] {
...
export function flattenedCandidates(image: RasterImage, rotation: Rotation, textBoxes: TextItemBox[]): ImportCandidate[] {
  ...
  return boxesOf(image).map((box) => {
    ...
    return { rects: [rect], text: intersectText(boxes, [rect]), extraction: "text_layer" as const, confidence: 1 }
  })
}
 ...
 */
export async function scanCandidates(image: RasterImage, source: CanvasImageSource, rotation: Rotation, recognize: RegionRecognizer = recognizeRegion): Promise<ImportCandidate[]> {
  const size = { w: image.width, h: image.height }
  return Promise.all(
    boxesOf(image).map(async (box) => {
      ...
      return { rects: [normalizeRect(crop, size, rotation)], text, extraction: "ocr" as const, confidence }
    })
  )
}
