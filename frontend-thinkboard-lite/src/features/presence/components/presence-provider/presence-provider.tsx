...
export function PresenceProvider({ children }: { children: ReactNode }) {
  const { profileId, sessionId, page } = useWorkspaceContext()
  ...
  const { peers, leaderDrawing, publishCursor, subscribe } = usePresence({ sessionId: sessionId ?? "", profileId: profileId ?? "", page, leaderId })
