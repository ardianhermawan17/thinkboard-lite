...
**Web identity and accessibility (2026-09-26, task 037).** A **jev-ultrafast** Lighthouse run on the live workspace scored **Accessibility 0.89 / Best Practices 1.00 / SEO 0.40** with six failing audits. Two were identity: no `<title>` and no meta description, and `/robots.txt` was invalid (the dev server served the app shell there). One was the mode switch: it was built from Radix **Tabs with no `TabsContent`**, so every trigger's `aria-controls` pointed at an element that does not exist — it is a segmented control, now a toggle group (`role="group"` + `aria-pressed`). The others: the Notes heading was an `h3` after an `h1`, and the rotate control's visible glyph was outside its accessible name. Fixed, plus a `manifest.webmanifest` so the tablet app installs standalone. Re-audited: **Accessibility 1.00 / Best Practices 1.00 / SEO 1.00, zero failing audits.**

**Sign-in screen (2026-09-26, task 038).** The first screen was a centred card that said nothing about the product. It is now a two-column screen: a brand side drawing the workspace's own vocabulary (a page with a highlight sweeping across a line, a teammate's cursor drifting after it, a note chip, a "2 here" pill) over two token glows, and the form beside it with real labels, `autocomplete`, show/hide password, a pending state and a `role="alert"` error. The brand side is `lg`-only so the form is never below the fold; exactly one `h1` keeps the outline correct; the state lives in the hook (I2) and the motion is task 036's transform-only seam. Audited on the page: **1.00 / 1.00 / 1.00, no failing audits**, and a real sign-in still opens the workspace.

**UI rework (2026-09-26, task 039).** A whole-app design pass, worked at the token layer so every surface moved together. Design read: *a local-first collaborative PDF-review workspace for a small tablet pilot, calm high-end technical language, soft structuralism on the existing shadcn + Tailwind v4 system*. The biggest single fix was colour: the primary was a **saturated violet**, the most recognisable AI-default accent, now a desaturated **petrol** that does not fight the yellow/green/blue/rose highlight swatches. Borders became **alpha-ink hairlines**, radii follow one documented scale, shadows are soft and tinted, easing is `cubic-bezier(0.16,1,0.3,1)` (no `ease-in-out`/`linear` anywhere), Lucide is thinned to 1.5 globally, and there is a single fixed `pointer-events-none` grain layer that disappears under `prefers-reduced-transparency`. The sign-in and new-workspace screens were then built out properly (eyebrow, display type, a double-bezel motif, a button-in-button pill CTA), with the header, notes panel (and a composed empty state), members (initials + role chip), page controls, presence rail and offline banner following. The re-audit caught one regression — `⟳ Rotate` failed `label-content-name-mismatch` because axe counts a text glyph in the visible label — now aria-hidden icons. Both pages finish at **Accessibility 1.00 / Best Practices 1.00 / SEO 1.00, zero failing audits**, and the gate stays green (374 / 14).

**PWA (2026-09-26, task 040).** The app now installs and opens offline. Four icons were generated from the product's own motif (a page with a highlight swept across a line), the manifest names them with a stable `id`, and iOS gets `appleWebApp` + an apple-touch-icon. The offline half needed three corrections that only a production build exposes: documents are **not** in the build's asset manifest, so the shell and the offline page are named in `additionalPrecacheEntries`; `fallbacks` does **not** precache its entries and only runs for a handler that matched, so an uncached navigation fell through to the browser's error; and **Serwist never disables navigation preload** — the setting lives on the registration, and the failed preload response is what painted a blank offline page, so the worker now disables it explicitly on activate. The navigation fallback is the **shell**, not an offline page, because the app is client-rendered: a deep link opened offline boots the shell and the router renders the workspace from Dexie. Proven with the production server killed — `/w` and `/w/f05e17f9-…` both render the app.

**First-run tour (2026-09-27, task 041).** The main feature now explains itself. A seven-step tour overlays the document view — the document, the tools, the notes, importing an already-highlighted PDF, the people, the export — with a spotlight that moves between the real elements and a card per step. The hole is a `clip-path: polygon(evenodd, …)` so the spotlit control stays clickable while the rest is dimmed; CSS transitions the hole and Motion springs the ring and pops the card. The flag lives on the workspace slice **outside `ui`** (`stripUi` would erase it), so it persists, and finishing or skipping both close it for good. Two things the live pass taught: the PDF canvas mounts **after** the workspace, so the first measurement gave up too early and left the spotlight at 0 (it now keeps looking), and a hidden tab **freezes CSS transitions and Motion springs alike**, so the proof reads inline styles rather than animation progress.

**The landing list (2026-09-27, task 042).** `/w` was a dead end of a specific kind: it redirected to the last workspace (RULE-14) or offered **create**, so anyone on a new device — or after signing out — had to be handed a URL to reach a workspace they already had. It now lists the workspaces the account can open, as rows above the create form, and the heading adapts ("Your workspaces" vs "Start a workspace"). The read is `select id, title` with **no client filter**: RLS scopes `sessions` to the caller's teams, so what you may read *is* your list, and a client-side filter would be a weaker second copy of a rule the database already enforces. It is a read only — nothing writes `meta` or the outbox, so it cannot park — and a failed load is silent, leaving exactly the old screen so that creating a workspace still works offline. The live suite gained **g5**, which proves the scoping against the real stack: the leader sees the workspace they created; a member of the seeded team sees the seeded workspace and not that new team's.
