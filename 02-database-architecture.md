...
extends: ["thinkboard-schema-final.sql", "01-thinkboard-schema-rationale.md", "01-thinkboard-lite-spec.md"]
companion: ["06-whole-apps-task.md", "06-whole-apps-task.html", "todo-task-000-split.md"]
supersedes: ["01-thinkboard-lite-spec.md §6 (SQL)", "05-frontend-sync-handwriting.md §5 (SQL addendum)"]
---

> **rev 1.1 (2026-09-18).** Reviewed against every plan doc. §8.1 is now the one, final `0004_lite.sql`,
> with C6 and the ten review fixes in §11 applied. Implement from §8.1, not from spec §6.

# 02 — Database Architecture (ThinkBoard Lite)
...
`06-whole-apps-task.md` exist exactly to unblock task 008 onward.
...
§8.1 is the final SQL. It supersedes `01-thinkboard-lite-spec.md` §6 and v2's §5 addendum. What it does, and why
each piece is there:
...
**Deliberately unchanged** (as spec §6): `highlights.bbox` absorbs all geometry; `highlights.extraction`
...
Every item below was found by reading this file against `01-thinkboard-lite-spec.md`, v2, both frontend docs and
`06-whole-apps-task.md`. **Applied** means §8.1 already contains the fix; its test is in §9.

| # | Defect | Status |
|---|---|---|
| DB-F1 | `note_input_mode` had two values in spec §6 and three in v2 §5; `transcribed_by` existed only as a later `alter` (conflict C6). | applied — three values, column in the `create table` |
...
| DB-F3 | spec §6 cannot run twice (`create type`, `add column`, `create policy`), yet task 001's gate requires a re-runnable `0004`. | applied — every statement guarded |
