...
extends: ["02-working.md", "04-TODO.md", "05-agent-limitation.md"]
companion: ["07-database-architecture.md", "07-whole-apps-task.html"]
---

# 07 — Whole-App Task Plan
...
| | |
|---|---|
| **ID** | `NNN-task-<slug>`, global monotonic per `02-working.md` §3. Never reused. |
...
goal id, and no git write without explicit confirmation (`05-agent-limitation.md` §1–§2).
...
**Mini-goals**
- `g1` Write `architecture.blueprint.json`: domain tables from `07-database-architecture.md`, contexts, the 4 commands, canvas leaves, listeners, `localFirst` block.
- `g2` Write `AGENTS.md` — dependency direction, the leaf convention, the 4-point store edit, the "ask, don't invent" rule.
- `g3` Extend `scripts/verify-architecture.mjs` with I11–I29 from `frontend-folder-architecture.md` §12 and the v2 additions.
...
- `g6` **Run the device test** from `01-thinkboard-lite-frontend-v2.md` §3.4 on the pilot's real tablets, in Indonesian, and record the result in `validate.json`.
...
5. **No git write without explicit confirmation, every time** — `05-agent-limitation.md` §1. Work lands on the shadow branch, never on `master`.
